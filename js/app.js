...
function toast(msg, type) {
  const tEl = $('#toast');
  tEl.textContent = msg;
  tEl.classList.toggle('err', type === 'err');
  tEl.hidden = false;
  clearTimeout(toastTimer);
  toastTimer = setTimeout(() => { tEl.hidden = true; }, 3000);
}
...
function typeLabel(type) {
  return { entrada: t('entry'), salida: t('exit'), ausencia: t('absence') }[type] || type;
}
...
function applyBrand() {
  const name = state.settings.app_name || t('brand_default');
...
function statusFor(userId) {
  ...
  if (!recs.length) return { text: t('status_none'), cls: 'none' };
  const last = recs[recs.length - 1];
  if (last.type === 'salida') return { text: t('exit') + ' ' + last.timestamp.slice(11, 16), cls: 'salida' };
  if (last.type === 'entrada') return { text: t('entry') + ' ' + last.timestamp.slice(11, 16), cls: 'entrada' };
...
function renderMarking() {
  ...
  if (!state.users.length) {
    grid.innerHTML = '<p class="muted">' + esc(t('user_empty')) + '</p>';
    lastGridKey = gridKey();
    return;
  }
...
async function submitPin() {
  ...
  try {
    ...
    if (state.pickMode === 'admin') {
      ...
      else toast(t('toast_admin_required'), 'err');
...
async function openPick(mode) {
  ...
  $('#pick-title').textContent =
    mode === 'admin' ? t('pick_admin_text') : t('pick_profile_text');
  const list = $('#pick-list');
  list.innerHTML = '<p class="muted">' + esc(t('pick_loading')) + '</p>';
  let candidates = [];
  try {
    if (mode === 'admin') {
    ...
    list.innerHTML = '<p class="error">' + esc(t('pick_error')) + '</p>';
    openModal('pick-modal');
    return;
  }
  list.innerHTML = '';
  if (!candidates.length) {
    list.innerHTML = '<p class="muted">' + esc(mode === 'admin' ? t('pick_none_admin') : t('pick_none_users')) + '</p>';
  } else {
...
async function uploadMyPhoto() {
  ...
  if (!file) return toast(t('toast_photo_select'), 'err');
...
async function changePin() {
  ...
  if (!newPin || newPin.length < 4) return toast(t('toast_pin_min'), 'err');
  if (newPin !== confirm) return toast(t('toast_pin_mismatch'), 'err');
  try {
...
async function submitRecovery() {
  ...
  if (!contact) return toast(t('toast_recovery_input'), 'err');
  try {
    const data = await api('/api/auth/recover', {
      method: 'POST',
      body: JSON.stringify({ contact }),
    });
    $('#rec-info').textContent = t('toast_code_sent') + (data.sentTo || '');
...
async function submitReset() {
  ...
  if (!code) return toast(t('toast_code_input'), 'err');
  if (!newPin || newPin.length < 4) return toast(t('toast_pin_min'), 'err');
  if (newPin !== confirm) return toast(t('toast_pin_mismatch'), 'err');
  try {
...
async function loadDashboard() {
  ...
  $('#hour-bars').innerHTML = visible.length
    ? visible
        ...
        .join('')
    : '<p class="muted">' + esc(t('no_entries_today')) + '</p>';
        ...
        .map(
          (u) =>
            `<tr>
              ...
              <td>${esc(u.email || '-')}</td>
              <td>${esc(u.phone || '-')}</td>
              <td><span class="chip ${u.role}">${u.role === 'admin' ? t('role_admin') : t('role_user')}</span></td>
              <td class="td-actions">
                <button class="btn btn-sm" onclick="editUser(${u.id})">${t('edit')}</button>
                <button class="btn btn-sm" onclick="delUser(${u.id})">${t('delete')}</button>
              </td>
            </tr>`
        )
        .join('')
    : '<tr><td colspan="6" class="muted">' + esc(t('users_table_empty')) + '</td></tr>';
}
...
function newUser() {
  editingUserId = null;
  $('#uf-title').textContent = t('user_form_new');
  $('#uf-id').value = '';
  $('#uf-name').value = '';
  $('#uf-email').value = '';
  $('#uf-phone').value = '';
  $('#uf-pin').value = '';
  $('#uf-pin').placeholder = t('key_placeholder');
  $('#uf-role').value = 'user';
  $('#uf-photo').value = '';
...
function editUser(id) {
  ...
  if (!u) return;
  editingUserId = id;
  $('#uf-title').textContent = t('edit') + ': ' + u.name;
  $('#uf-id').value = u.id;
  $('#uf-name').value = u.name;
  $('#uf-email').value = u.email || '';
  $('#uf-phone').value = u.phone || '';
  $('#uf-pin').value = '';
  $('#uf-pin').placeholder = t('key_empty_placeholder');
...
async function saveUser() {
  ...
  if (!payload.name) return toast(t('toast_name_required'), 'err');
  try {
    if (editingUserId) {
      await api('/api/users/' + editingUserId, { method: 'PUT', body: JSON.stringify(payload) });
    } else {
      if (!payload.pin) return toast(t('toast_initial_key'), 'err');
      await api('/api/users', { method: 'POST', body: JSON.stringify(payload) });
    }
    closeModal('user-form-modal');
    toast(t('toast_user_saved'));
...
async function uploadUserPhoto() {
  ...
  if (!id) return toast(t('toast_user_first_save'), 'err');
...
async function fillHistoryUsers() {
  ...
  sel.innerHTML =
    '<option value="">' + esc(t('all_users')) + '</option>' +
...
async function exportCSV() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(data.error || t('toast_csv'));
    }
...
async function uploadLogo() {
  ...
  if (!file) return toast(t('toast_file_select'), 'err');
  const fd = new FormData();
  fd.append('file', file);
  try {
...
function setupInstall() {
  ...
  if (standalone) {
    ...
    $('#btn-install').hidden = false;
    $('#install-hint').textContent = t('ios_hint');
  }
}
...
function resetInactivity() {
  ...
  if (state.token) {
    inactivityTimer = setTimeout(() => {
      headerLogout();
      toast(t('toast_session_timeout'), 'err');
    }, INACTIVITY_TIMEOUT);
  }
}
...
(async function init() {
  const savedLang = localStorage.getItem('ca_lang') || 'es';
  setLanguage(savedLang);

  try {
    await loadSettings();
  } catch (e) {
    toast(t('toast_server_error'), 'err');
  }
