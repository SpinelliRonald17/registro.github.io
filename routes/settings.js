...
const PUBLIC_KEYS = ['app_name', 'default_theme', 'accent', 'logo', 'background', 'language'];

router.get('/public', (req, res) => {
  const rows = db.prepare(
    "SELECT key, value FROM settings WHERE key IN ('app_name', 'default_theme', 'accent', 'logo', 'background', 'language')"
  ).all();
  const settings = {};
  rows.forEach((r) => {
...
router.put('/', authRequired, adminRequired, (req, res) => {
  ...
  if (body.language && /^[a-z]{2}$/.test(body.language)) {
    set.run('language', body.language);
  }
  res.json({ ok: true });
});
